...
def About():
    top2 = Toplevel()
    top2.title("About Weather Forecast App")
    top2.geometry('400x300')
    top2.config(bg="SteelBlue2")

    info = Label(top2,text="Developed By : Purvin Setia\n\nContact Info : purvinsetia97@gmail.com")
...
def Forecast_Window():
    ...
    top.config(bg="SteelBlue2")

    city = Label(top,text=cit.upper())
    city.place(relx=0.07,rely=0.13,anchor=NW)
    city.config(bg="white",fg="SteelBlue2",font=("Courier 17 bold"))
    ...
    head = Label(top,text='WEATHER FORECAST')
...
def future_forecast():
    forecast = Label(root,text="Weather Forecast For \nNext 3 Days")
...
def showtemp(wf):
    ...
    cityna.place(relx=0.5,rely=0.12,anchor=N)
    cityna.config(bg="white",fg="SteelBlue4",font=("Courier 17 bold"))

    date = Label(root,text=datetime.datetime.now().strftime("%d %b %Y"))
    date.place(relx=0.5,rely=0.20,anchor=N)
    date.config(bg="white",fg="SteelBlue4",font=("Courier 13 bold"))
    
    temp = Label(root,text="Temperature\n%s'C"%wf['T'])
    temp.config(bg="ghost white",fg="SteelBlue4",font=("Courier 12 bold"))
    temp.place(relx=0.5,rely=0.5,anchor=CENTER)

    humi = Label(root,text="Humidity : {}%".format(wf['H']))
    humi.config(bg="ghost white",fg="steel blue",font=("Courier 14 bold"))
    humi.place(relx=0.08,rely=0.7,anchor=W)

    pres = Label(root,text="Pressure : %d hPa"%wf['P'])
    pres.config(bg="ghost white",fg="steel blue",font=("Courier 14 bold"))
    pres.place(relx=0.08,rely=0.8,anchor=W)

    wsp = Label(root,text="Wind Speed : %d m/s"%wf['WS'])
    wsp.config(bg="ghost white",fg="steel blue",font=("Courier 14 bold"))
    wsp.place(relx=0.6,rely=0.7,anchor=W)
   
    wdr = Label(root,text="Wind Direction : %d'"%wf['WD'])
    wdr.config(bg="ghost white",fg="steel blue",font=("Courier 14 bold"))
    wdr.place(relx=0.6,rely=0.8,anchor=W)

    wds = Label(root,text="Weather Description : %s"%wf['WDS'])
    wds.config(bg="ghost white",fg="steel blue",font=("Courier 14 bold"))
    wds.place(relx=0.25,rely=0.9,anchor=W) 
...
head = Label(root,text='WEATHER FORECAST')
head.config(bg="White",fg="black",font=("Courier 22 "))
head.place(relx=0.5,rely=0,anchor=N)
